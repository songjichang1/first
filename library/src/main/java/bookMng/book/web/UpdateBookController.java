...
/* Method Name  :  UpdateBookController  
 * ==================================== Method Detail =========================================
 * 1. UpdateBookPage.JSP에서 특정 도서의 정보를 수정하기 위해 전달 받은 값을 처리 할 bs.updateBook(bd) 전달.
...
public class UpdateBookController  implements Controller{
	...
	public String rquestHandler(HttpServletRequest request, HttpServletResponse rseponse)
		...
		// BookService에게 값을 전달하기 위해 BookDTO 선언
		BookDTO bd = new BookDTO();
		//FrontController(Dispatcher-Servlet)에서 전달받은 값을 처리 할 service를 호출하기 위해 객체 생성
		BookService  bs = new BookService();
		
		/*
		 * UpdateBookPage.JSP에서 input 태그에서 name에 적은 이름을 getParameter(); ()안에 동일하게 넣는다.
